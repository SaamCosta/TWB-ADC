help_file = {
    'server': 'Configure the world the bot should play on',
    'server.server': 'The server endpoint to use (the world)',
    'server.endpoint': 'Endpoint server to use (the full url)',
    'server.server_on_twstats': 'Is the server listed on twstats.com?',
    'reporting': 'Enable advanced reporting features',
    'reporting.enabled': 'Enable the reporting feature',
    'reporting.connection_string': 'Could be of type file://filename or mysql://user:password@host:port/database_name',
    'notifications': 'Push notifications over Telegram for events that need a human '
                     '(bot protection / captcha, village under attack, crashes)',
    'notifications.enabled': 'Enable Telegram notifications. Requires the python-telegram-bot '
                             'package plus a token and channel id below',
    'notifications.token': 'Telegram bot token, from @BotFather. Treat it as a credential — it is '
                           'why config.json must never be committed',
    'notifications.channel_id': 'Telegram chat/channel id the notifications are sent to',
    'bot': 'Set global bot configuration variables',
    'bot.active_hours': 'The hours when the bot should be active, e.g. "6-23" (does not impact attack timings)',
    'bot.check_update': 'Check GitHub for a newer version of the bot on start-up (does not install anything)',
    'bot.delay_factor': 'Delay factor to use — 5-7 seconds * delay factor (very low factors will probably cause ban)',
    'bot.active_delay': 'Delay in seconds to use during active hours',
    'bot.inactive_delay': 'Delay in seconds to use during inactive hours',
    'bot.inactive_still_active': 'When disabled, stops the bot from running during inactive hours',
    'bot.add_new_villages': 'Automatically add the default village config to newly conquered villages',
    'bot.village_name_template': 'Template to use for new villages — use {num} to set the config index as name',
    'bot.village_name_number_length': 'The number length — lower values will be prefixed with zeroes',
    'bot.auto_set_village_names': 'Automatically set village names based on the template',
    'bot.user_agent': 'Set this to the browser agent your session is using (mismatch may cause ban)',
    'bot.max_cached_reports': 'Maximum number of files kept in cache/reports — the oldest are pruned once the limit is exceeded. Keeps the per-cycle farm/report cross-check bounded. 0 disables pruning',
    'bot.humanize_village_order': 'Shuffle the village processing order every cycle instead of always following the same sequence (Feature 23)',
    'bot.jitter_min': 'Lower bound (seconds) of the random jitter added to every cycle delay (Feature 23)',
    'bot.jitter_max': 'Upper bound (seconds) of the random jitter added to every cycle delay (Feature 23)',
    'bot.attention_lag_chance': 'Probability (0.0-1.0) per cycle of an extra-slow "distracted player" delay. 0 disables it. Never triggers while a managed village is under attack (Feature 23)',
    'bot.attention_lag_extra_min': 'Minimum extra seconds added to the cycle delay when the attention-lag roll succeeds (Feature 23)',
    'bot.attention_lag_extra_max': 'Maximum extra seconds added to the cycle delay when the attention-lag roll succeeds (Feature 23)',
    'building': 'Automatic construction of buildings',
    'building.manage_buildings': 'Automatically manage buildings',
    'building.default': 'The default build template to use — village configs override this',
    'building.max_lookahead': 'Max number of items in queue to check before stopping',
    'building.max_queued_items': 'Max queued items (default: 2, premium: 5)',
    'building.auto_queue_len': 'When enabled and world.premium_account is detected as true, automatically use building.premium_max_queued_items instead of building.max_queued_items (Feature 22)',
    'building.premium_max_queued_items': 'Queue length used instead of max_queued_items when auto_queue_len is enabled and the account is premium (Feature 22)',
    'units': 'Automatic recruitment and upgrade of units',
    'units.recruit': 'Automatically recruit units',
    'units.upgrade': 'Automatically upgrade units (level 1-3 and 1-10 smith systems)',
    'units.default': 'Default template for unit recruitment (templates/troops)',
    'units.batch_size': 'Amount of units to attempt to create per run — increase in late-game',
    'units.manage_defence': 'Manage defence distribution between villages (experimental)',
    'units.remove_manual_queued': 'Remove manually queued recruitment entries',
    'units.randomize_unit_queue': 'Randomize unit queue for a wider variety of units',
    'farms': 'Automatic farming of nearby (barbarian) villages',
    'farms.farm': 'Enable automatic farming',
    'farms.min_points': 'Minimum points of villages to attack',
    'farms.max_points': 'Maximum points of villages to attack',
    'farms.search_radius': 'Max radius of villages to attack (fields)',
    'farms.map_sector_radius': 'Extra map sectors (20x20 fields each) fetched around every village '
                               'via map.php. 0 = only what the map screen preloads, which is 2 '
                               'sectors and is not centered on the village. 1 = a 3x3 sector block '
                               '(60x60 fields), 2 = 5x5. More sectors means more farm targets, at '
                               'the cost of one bigger map request per village every 8 hours.',
    'farms.default_away_time': 'Default cooldown in seconds before attacking a village again',
    'farms.full_loot_away_time': 'Cooldown for villages with high resource gain',
    'farms.low_loot_away_time': 'Cooldown for villages with low resource gain',
    'farms.high_profile_fill_rate': 'A farm is marked high profile (revisited on the shorter '
                                    'full_loot_away_time) when this fraction of its attacks came '
                                    'back with the pack completely full. Fill rate is used instead '
                                    'of average loot because loot is capped by the pack you send, '
                                    'so an absolute threshold silently drifts whenever the troop '
                                    'templates change.',
    'farms.low_profile_utilization': 'A farm is marked low profile (revisited on the longer '
                                     'low_loot_away_time) when it returns on average less than this '
                                     'fraction of the capacity sent to it. A farm that rarely fills '
                                     'a large pack but still returns most of it is not poor, so '
                                     'this uses utilization rather than fill rate.',
    'farms.profile_min_attacks': 'Minimum number of attacks on a farm before it can be classified '
                                 'as high or low profile',
    'farms.max_farms': 'Number of nearby villages to check for farming',
    'farms.attack_higher_points': 'If disabled, villages with higher points than the current one are ignored',
    'farms.force_scout_if_available': 'Only attack villages that have been attacked before or scouted first',
    'farms.farm_scout_amount': 'Number of spies used to scout a village before farming',
    'farms.forced_peace_times': 'Windows during which the bot must not farm (national holidays, '
                                'forced peace events). List of {"start": ..., "end": ...} with the '
                                'format "dd.mm.yy HH:MM:SS" — a malformed entry is skipped with a '
                                'warning instead of stopping the cycle',
    'market': 'Automatic management of public market trading (offer system)',
    'market.auto_trade': 'Enable automated public market trading',
    'market.max_trade_duration': 'Max duration of public trade offers (hours)',
    'market.auto_remove': 'Automatically remove existing or expired trade offers',
    'market.trade_multiplier': 'Enable uneven trade ratios (if supported by the world)',
    'market.trade_multiplier_value': 'Trade multiplier value — lower means more gain',
    'market.trade_max_per_hour': 'Max number of trades the bot can do in 1 hour',
    'premium_exchange': 'Selling resources to the premium exchange for Premium Points (Feature 34). Gated by world.trade_for_premium AND the per-village trade_for_premium flag — this section only tunes HOW it sells, never whether it runs',
    'premium_exchange.max_rate_per_point': 'Only sell while the exchange asks at most this many resources for 1 Premium Point. Lower is stricter. The floor early in a world is around 66, rises to ~90 over the first days and past 800 once the exchange stock fills up — see docs/backend.md',
    'premium_exchange.sell_batch': 'Resources sold per order. Small lots matter: the rate degrades within your own sale, so 6000 at once is worth less than six lots of 1000',
    'premium_exchange.min_sell_batch': 'Do not sell a lot smaller than this. A merchant carries 1000 and takes 2 hours round trip, so shipping 200 in one wastes the resource the strategy calls its own bottleneck. Set below sell_batch only if you would rather sell partial lots than wait',
    'premium_exchange.max_batches_per_run': 'Max lots sold per village cycle. Each lot re-reads the exchange, so the next one uses the fresh rate instead of assuming the previous one',
    'premium_exchange.min_free_merchants': 'Do not sell unless at least this many merchants are free. Selling uses your own village merchants and delivery takes 2 hours',
    'premium_exchange.keep_wood': 'Never sell wood below this amount (on top of what building/recruiting already reserved)',
    'premium_exchange.keep_stone': 'Never sell clay below this amount (on top of what building/recruiting already reserved)',
    'premium_exchange.keep_iron': 'Never sell iron below this amount (on top of what building/recruiting already reserved)',
    'resource_sharing': 'Automatic direct resource transfer between your own villages (Feature 9)',
    'resource_sharing.enabled': 'Enable automatic resource sharing between managed villages',
    'resource_sharing.priority': 'Priority mode for need-based transfers — "new_villages" sends to the lowest-point villages first, anything else sends to the largest deficit first',
    'resource_sharing.max_sends_per_cycle': 'Max number of separate transfers a single village may start per cycle',
    'resource_sharing.min_send_amount': 'Transfers smaller than this (per resource) are not worth a merchant trip and are skipped',
    'resource_sharing.receiver_fill_max_pct': 'Never fill a receiving village above this percentage of its storage — leaves margin for the production that keeps running during travel time',
    'resource_sharing.merchant_capacity': 'Resources carried per merchant (1000 on worlds without a merchant bonus — check <MerchantBonus> in the public world config)',
    'resource_sharing.overflow_enabled': 'Rule 1 — rescue resources from a village whose warehouse is about to overflow',
    'resource_sharing.overflow_trigger_pct': 'A resource above this percentage of the village own storage counts as overflowing (e.g. 85 = 85%)',
    'resource_sharing.overflow_target_pct': 'When rescuing an overflow, send away enough to bring the resource back down to this percentage',
    'resource_sharing.need_enabled': 'Rule 2 — supply villages that are blocked on resources for their build/recruit queue',
    'resource_sharing.need_donor_floor': 'Absolute amount of a resource a village keeps for itself before donating under rule 2. Absolute on purpose — a percentage of own storage makes big-warehouse villages permanently ineligible to donate',
    'conquest': 'Automated barbarian village conquest (Feature 8)',
    'conquest.enabled': 'Enable automated conquest — activate only when nobles are available',
    'conquest.max_radius': 'Maximum radius in fields to search for conquest targets',
    'conquest.priority': 'Target selection priority — "fill_gaps" scores by centrality, distance and points',
    'conquest.min_points': 'Minimum points a target village must have to be considered',
    'conquest.max_points': 'Maximum points a target village can have to be considered',
    'conquest.area_of_interest': 'Coordinate box the conquest should grow into. It is a PREFERENCE, not a filter: a target inside the box always beats one outside, but when nothing inside is eligible the bot falls back to the full candidate list instead of idling. Needed because "fill_gaps" scores by centrality to your own empire, so it densifies the middle of the cluster and never elects the frontier — the box is what says which way to expand. br143/SQUAD 02: 500-650 | 270-299 (southern K25 plus half of K26)',
    'conquest.area_of_interest.enabled': 'Enable the area preference — off restores pure "fill_gaps" scoring',
    'conquest.area_of_interest.x_min': 'Left edge of the area, inclusive',
    'conquest.area_of_interest.x_max': 'Right edge of the area, inclusive',
    'conquest.area_of_interest.y_min': 'Top edge of the area, inclusive — lower Y is further north',
    'conquest.area_of_interest.y_max': 'Bottom edge of the area, inclusive',
    'conquest.loyalty_regen_per_hour': 'Loyalty regeneration per hour. Per-world value published at /page/settings ("Aumento de lealdade por hora") — read it there, do not guess. br143 = 1',
    'conquest.loyalty_drop_per_noble': 'FALLBACK ONLY — the bot reads the real range from the world itself (<mood> loss_min/loss_max in interface.php?func=get_config, same numbers /page/settings shows as "Redução de lealdade por ataque do nobre"; br143 = 20-35) and plans against the worst case. This key is only used when the world did not publish the block. Note the drop is a RANGE, not a value: a 4-noble train removes 80 to 140, not a guaranteed 100',
    'conquest.escort_ratio': 'Fraction of available troops sent as noble escort. WARNING — despite living in the "conquest" (barbarian) section, this key is ALSO read by the PvP conquest (game/pvp_conquest.py) for its noble escorts. Against a player a thin escort lets the defender snipe a single command out of the train and lose the noble, so do not lower this to save troops on barbarians — use barbarian_escort_ratio for that',
    'conquest.barbarian_escort_ratio': 'Fraction of available troops sent as escort for BARBARIAN noble trains only. Leave unset (or null) to fall back to escort_ratio. Exists because a barbarian has no defender and no owner, so a large escort is mostly waste — the troops stay out of the village for the whole round trip without farming — while the PvP escort has the opposite requirement. min_escort_total still applies as the floor',
    'conquest.min_escort': 'Minimum escort units per unit type (optional override)',
    'conquest.min_escort_total': 'Minimum combined escort troops per noble attack, regardless of unit type (default: 50)',
    'conquest.escort_reserve_max_pct': 'Maximum fraction of any single unit type that may be reserved while waiting for the escort to build up (default: 0.8). Below 1.0 a residual always stays available to farm and gather — 1.0 restores the old behaviour of reserving up to 100% of a type',
    'conquest.escort_reserve_min_progress': 'Only start reserving troops for the escort once this fraction of the required total is already home (default: 0.5). Far from the target the reserve just freezes the farm income that funds recruitment, so it is skipped — 0.0 restores the old always-reserve behaviour',
    'conquest.respect_tribe_reservations': 'Read the tribe\'s official reservation board (screen=ally&mode=reservations) and never conquer a village someone else has reserved (default: true). Applies to automatic targets, manually queued ones, conquests already under way and PvP conquest. If the board cannot be read the bot does not START a new conquest — without it there is no way to know what belongs to whom, and the cost is asymmetric: skipping a free target costs one barbarian among dozens, while nobling someone else\'s reservation costs social capital and cannot be undone. Conquests already under way are NOT aborted on a read failure, because that would throw away troops already in the air',
    'conquest.reservation_cache_seconds': 'How long the reservation board is cached before being re-read (default: 600). A third party\'s reservation can appear at any moment and re-reading costs one request; a stale board is served for up to 6x this value when a refresh fails, so a network hiccup does not freeze conquest entirely',
    'conquest.reserve_targets': 'Phase 2 of the reservation system: when the bot schedules a barbarian noble train, also CREATE a reservation for that target on the alliance board, and release it once the conquest resolves (default: false). This writes to a board shared with every human in the alliance, which is why it has its own gate and ships off. Reading the board (respect_tribe_reservations) is purely defensive and unrelated; this is the opposite direction — instead of the bot quietly stepping on the tribe, it announces what it is going to take. The reservation is created only AFTER the train is scheduled, never while a target merely sits in the manual queue: measured on 2026-09-21, the window between scheduling and conquering was 7.2h and 10.7h against the alliance validity of 3 days, so nothing ever needs renewing. Only reservations the bot itself created (identified by reserve_comment, read back from the game) are ever removed — a reservation you made by hand is never touched, and one of unknown provenance is left to expire on its own',
    'conquest.reserve_max_slots': 'How many of the account\'s reservation slots the bot may occupy at once (default: 1). The alliance grants 5 per player and that setting belongs to the alliance, so it is never written from here — this is the bot\'s own ceiling inside it. The default is 1 because the planner keeps exactly one barbarian train in flight empire-wide, so one slot is provably enough; measured on 2026-09-21 the account already held 2 slots taken by hand, and without a ceiling the bot would fill the board and leave you unable to reserve anything, with no error — just refusal. A reservation whose provenance cannot be established counts toward this ceiling (so the bot creates less), but is never removed',
    'bot.reuse_game_data_max_age': 'Seconds (default: 60; 0 turns it off). Before recruiting and after the market, the bot used to re-read the village overview just to refresh resources and farm space. Every HTML screen of the game already carries the same game data, so when the last screen the bot opened for that village is at most this many seconds old, it reuses that instead of spending a request. Decided on the full daytime cycle of 2026-09-23 (docs/backend.md §9 item 20a): 67 of 67 comparisons at these two points showed nothing beyond the production of the interval, with the previous data at most 34 s old. About 67 requests per cycle. Older data, or data from an AJAX/JSON response, falls back to the old request',
    'conquest.snipe_expiring_reservations': 'Reservation timer (docs/backend.md 8.28, default: false). For every target in the manual conquest queue that is reserved by someone else on the alliance board, the bot reads the expiry ("hoje às 17:31", "amanhã às …", "em 24.09. às …") and reserves the target for this account in the minute it expires, retrying once a minute up to 8 times. It holds the slot before there are nobles for the train — the train reserves anyway when scheduled, but by then another ally may have taken it. Requires reserve_targets (it is a write) and counts against reserve_max_slots. If the owner renews the reservation the timer re-arms for the new time; if the village gets an owner it gives up. Queue targets with a bot reservation are no longer released by the end-of-conquest sweep',
    'conquest.reserve_comment': 'Comment written on every reservation the bot creates, and the stamp that proves the reservation is the bot\'s. It is VISIBLE TO THE ENTIRE ALLIANCE, so treat it as a message to other people rather than a log line. It has to be stable: the bot recognises its own reservations by comparing this exact text, so changing it makes every existing bot reservation look foreign — harmless but wasteful, since those will be left alone until they expire (3 days). Leave empty to use the default',
    'conquest.excluded_targets': 'Manual escape valve: list of village ids ("40808") or coordinates ("531|289") that must never be conquered, mixed freely in the same list. This is the override, not the solution — with the official board working it tends to stay empty. Use it for reservations agreed outside the game (forum, Discord) or any target you want to bar for another reason',
    'conquest.use_world_village_list': 'Discover conquest targets from the world\'s own public village list (map/village.txt) instead of only from what the empire has already scanned (default: true). Measured on br143 in 2026-09-20: the world has 130,909 villages, the shared cache/villages snapshot had 851 — a coverage of 0.65%, which made max_radius a discovery sieve disguised as a reach decision. The file needs no session and is read at most once every world_village_list_ttl seconds. It is the floor for DISCOVERY only: for OWNERSHIP the live map scan and this list both outrank cache/villages, because the local cache had 38 villages recorded as barbarian that the world already gave to players, against zero in the opposite direction. Turn off to restore the two-source pool',
    'conquest.world_village_list_ttl': 'How long the world village list is cached on disk before being downloaded again, in seconds (default: 21600, i.e. 6 hours). Ownership moves at the scale of conquests, not minutes. On any download failure the bot keeps using the expired copy rather than going blind, and warns',
    'inheritance': 'Config inheritance mode for newly conquered villages (Feature 6)',
    'inheritance.mode': 'Inheritance mode: "empire_ratio" (profile-aware), "nearest_village" (closest donor), "global_template" (original behaviour)',
    'empire': 'Empire-wide offensive/defensive village ratio (Feature 7). Villages with the "watchtower" profile are excluded from this count entirely (Feature 30)',
    'empire.offensive_ratio': 'Offensive side of the ratio. With offensive_ratio 1 and defensive_ratio 3 the empire targets 1 offensive village for every 3 defensive ones',
    'empire.defensive_ratio': 'Defensive side of the ratio (default 3, i.e. a defensive-leaning empire)',
    'player_stats': 'Account-wide loot/gather history read from the game\'s own '
                    '"screen=info_player&mode=stats_own" page (Feature 37). The game already '
                    'aggregates "Saqueado" (looted) and "Coletado" (gathered) resources per day, '
                    'server-side — this is the only source that sees what the player did by hand '
                    'outside the bot. Whole-account only, 7-day retention; does not replace '
                    'ReportReader or farmscores for per-village decisions. See docs/backend.md 8.13',
    'player_stats.enabled': 'Read the account-wide loot/gather series (default true — one cheap GET, no write, no gameplay effect)',
    'player_stats.cache_seconds': 'Minimum seconds between reads of stats_own (default 21600, i.e. 6 hours). The series resolution is daily, so re-reading every cycle would not change the numbers, only spend request budget',
    'in_flight': 'What is in the air RIGHT NOW, read from the game\'s own command overview '
                 '"screen=overview_villages&mode=commands" (Feature 38). Powers the "Em voo" '
                 'panel: every outgoing attack, return, support and withdrawal, with origin, '
                 'target, troops and the arrival time the SERVER calculated — never an estimate '
                 'from attack_duration(), which returns 0 on a parse failure and would make a '
                 'noble look already landed. Read-only observability: it changes nothing the bot '
                 'does. See docs/frontend.md 6.1.2 item 1',
    'in_flight.enabled': 'Read the command overview once per cycle (default true — one GET, no write, no gameplay effect)',
    'in_flight.cache_seconds': 'Minimum seconds between reads of the command overview (default 600). Short on purpose, unlike player_stats: this is the fastest-ageing data in the cache, because commands LAND. It is a ceiling against re-reading within one cycle, not a rhythm — the cycle interval is what actually paces it',
    'watchtower': 'Territorial allocation of watchtower villages (Feature 30). A watchtower village is defensive by nature but is allocated by geography, not by the empire ratio — so it is excluded from the empire count. Requires the world to actually have the watchtower building',
    'watchtower.enabled': 'Allow the bot to designate newly conquered villages as ADDITIONAL watchtower villages. The first tower is never created automatically — pick an established, central village by hand and set its profile to "watchtower". Leave false on worlds without the watchtower building',
    'watchtower.min_spacing': 'Minimum distance in tiles between two watchtower villages. A conquered village farther than this from every existing tower becomes a tower itself. Default 16, just above the level-20 radius of 15.0 tiles — larger values collapse the warning time at the seams (at 26 tiles, the worst point is tagged the instant the attack lands); see docs/backend.md',
    'watchtower.min_villages': 'Do not designate any watchtower village until the empire has at least this many villages. Guards a young account against converting an early village into a tower it cannot afford',
    'profile_templates': 'Per-profile config applied to every newly conquered village (Feature 7). These keys always override whatever was inherited from the donor village, so put here anything that *defines* the profile',
    'profile_templates.offensive': 'Keys forced on villages assigned the "offensive" profile',
    'profile_templates.defensive': 'Keys forced on villages assigned the "defensive" profile (e.g. support_others, so defensive villages send support by default)',
    'profile_templates.watchtower': 'Keys forced on villages assigned the "watchtower" profile (Feature 30)',
    'profile_templates.offensive.units': 'Unit template. Name the ROLE ("off") and the bot picks off_archer or off_no_archer from the world <game><archer> flag, every cycle. A literal file name still works and turns the automatic choice off',
    'profile_templates.defensive.units': 'Unit template. Name the ROLE ("def") and the bot picks def_archer or def_no_archer from the world <game><archer> flag, every cycle. Do NOT hardcode def_no_archer on an archer world: sword defense_cavalry is 15 there instead of 25, so that composition is silently much weaker and nothing warns you',
    'world': 'World-specific feature flags. archers_enabled, building_destruction_enabled and knight_enabled are now read from the world itself (interface.php?func=get_config) -- the values here are only a fallback for when that fetch fails',
    'world.knight_enabled': 'FALLBACK ONLY -- read from the world <game><knight> tag; used only if the world config cannot be fetched',
    'world.flags_enabled': 'Allows automatic management of flags (upgrading and defence). Still manual: the world config does not publish a flags tag',
    'world.flag_priority': 'Ordered flag preference for villages WITHOUT an academy, out of combat. '
                           'The bot equips the first type that is actually in the inventory, so the '
                           'tail exists to stop a village ending up with no flag at all. '
                           'Types: 1 production, 2 recruitment, 6 population, 8 loot',
    'world.flag_priority_academy': 'Same, for villages that have an academy (snob building level '
                                   '>= 1). Coin cost (7) comes first because coins are account-wide '
                                   'while production is local, so the discount is worth more where '
                                   'minting actually happens',
    'world.flag_manual_types': 'Flag types the bot never equips on its own and never removes from a '
                               'village that already has one — tactical picks left to the player '
                               '(3 attack, 5 luck). Defence (4) is deliberately NOT here: it is '
                               'automatic, but only through the under-attack path, and it overrides '
                               'even a manual flag',
    'world.quests_enabled': 'World has quests enabled (bot will automatically finish them)',
    'world.boosters_enabled': 'World has resource boosters enabled. Auto-detected on the first run '
                              'and written back here; a value set by hand always wins',
    'world.trade_for_premium': 'World has the premium market enabled (doing this too much could result in ban)',
    'world.archers_enabled': 'FALLBACK ONLY -- read from the world <game><archer> tag; used only if the world config cannot be fetched',
    'world.building_destruction_enabled': 'FALLBACK ONLY -- read from the world <build><destroy> tag; used only if the world config cannot be fetched',
    'world.premium_account': 'Auto-detected premium account status (from the overview page). Set manually to override detection (Feature 22)',
    'village_template': 'Default template applied to all newly managed villages',
    'village.building': 'Override build template for this village',
    'village.units': 'Override recruitment / farm template for this village',
    'village.managed': 'This village should be managed by the bot',
    'village.active_hours': 'Active hours override for this village (e.g. "8-22") — null uses global setting',
    'village.delay_factor': 'Request delay factor override for this village — null uses the global bot.delay_factor (Feature 23)',
    'village.keep_resources': 'Amounts this village keeps for itself before donating under the resource sharing need rule, e.g. {"stone": 30000} while saving for a noble. Raises resource_sharing.need_donor_floor for this village only; does not hold back overflow, which would be wasted anyway',
    'village.profile': 'Village combat profile: "offensive" or "defensive" — set automatically by empire ratio',
    'village.additional_farms': 'List of village IDs to include in the farming process',
    'village.prioritize_building': 'Do not recruit if the builder does not have enough resources',
    'village.prioritize_snob': 'Do not recruit if the noble queue does not have enough resources',
    'village.trade_for_premium': 'Trade left-over resources for premium points (requires global option)',
    'village.gather_enabled': 'Use left-over units to gather additional resources',
    'village.gather_selection': 'The gather operation to perform (must be unlocked first)',
    'village.gather_unlock_enabled': 'Automatically unlock scavenging options, cheapest pending one first, at most one per cycle. Only spends when the village can pay the full unlock_cost read from the game screen, and skips the village entirely while any unlock is already running (the game allows one at a time, which is also what keeps the bot from competing with manual unlocks). Respects keep_resources. Requires gather_enabled, since it rides on the same page request',
    'village.snobs': 'Number of nobles to maintain in this village',
    'village.mint_coins': 'Mint gold coins without ever recruiting a noble (only applies when snobs is 0). Coins belong to the account, nobles cost this village\'s population - this is what a watchtower village should do. Only mints from surplus - the priority order is buildings, then troops, then coins: it never mints while the build queue still lacks resources or while the village is below its troop template (support losses included), and it never requests resources from the market',
    'village.evacuate_fragile_units_on_attack': 'Automatically evacuate fragile units (axe, noble) to nearby safe villages when under attack',
    'village.support_others': 'Allow sending automatic support to allied villages',
    'village.support_others_factor': 'Fraction of defensive units to use in support operations',
    'village.support_others_max_villages': 'Max number of villages to send support to',
    'village.request_support_on_attack': 'Automatically request support units when under attack',
    'village.conquest_enabled': 'Enable conquest actions from this village — overrides global conquest.enabled',
    'village.evacuate_on_zone_attack': 'Trigger preemptive evacuation of fragile units (axe, noble) when zone neighbors are under attack — requires evacuate_fragile_units_on_attack: true (Feature 12)',
    'village.zone_attack_threshold': 'Minimum number of zone neighbors that must be under attack simultaneously to trigger preemptive evacuation (Feature 12)',
    'village.evacuate_urgency_threshold_sec': 'Only actually evacuate when the soonest incoming command is within this many seconds of arriving (default 1800 = 30 min) — farther-out commands just keep the defence flag active (Feature 16)',
    'village.zone_attack_eta_window_sec': 'Só conta um vizinho de zona como "sob ataque" para a evacuação preventiva se o ataque dele chegar dentro deste prazo (padrão 14400 = 4h). Sem isso, um fake com dias de viagem numa vizinha fazia esta aldeia despachar nobre e bárbaro por dias (Feature 12)',
    'village.support_lead_time_sec': 'Antecedência máxima para enviar apoio, somada ao tempo de viagem: o apoio sai quando viagem < ETA <= viagem + este valor (padrão 7200 = 2h). Não reutiliza evacuate_urgency_threshold_sec porque apoio precisa CHEGAR antes do impacto e ele mesmo leva horas viajando. Mantenha maior que o intervalo entre dois ciclos da mesma aldeia, senão o bot pula a janela',
    'hunter': 'Agendamento de ataques coordenados com chegada simultânea (Feature 10) — gerencie pela página /hunter',
    'hunter.enabled': 'Ativar o Hunter — quando true, o bot verifica e dispara os schedules a cada ciclo',
    'hunter.schedules': 'Exemplo do formato de um agendamento. O estado real vive em '
                        'cache/hunter/schedules.json e é gerenciado pela página /hunter — editar '
                        'aqui não agenda nada',
    'zones': 'Agrupamento geográfico de aldeias em zonas por proximidade (Feature 11)',
    'zones.enabled': 'Ativar o sistema de zonas geográficas',
    'zones.radius': 'Raio em campos para agrupar aldeias na mesma zona (padrão: 10)',
    'pvp_conquest': 'Agendamento semi-manual de conquistas PvP com scout + simulação + Hunter (Feature 13)',
    'pvp_conquest.enabled': 'Ativar o módulo de conquista PvP',
    'pvp_conquest.clear_ratio': 'Fração das tropas da aldeia ofensiva usada no clear (ex: 0.8 = 80%)',
    'pvp_conquest.min_attack_power': 'Poder de ataque mínimo (soma ATT × quantidade) para considerar o clear viável',
    'pvp_conquest.nobles_per_target': 'Número máximo de nobles a enviar por alvo',
    'pvp_conquest.arrival_buffer_seconds': 'Segundos de antecedência do clear em relação aos nobles',
    'pvp_conquest.scout_amount': 'PISO de espiões para uma aldeia poder ser origem do scout inicial — não é a quantidade enviada. A espia sai da aldeia com MAIS espiões (empate desfeito pela menor distância) e leva todos os que estiverem em casa, descontada qualquer reserva de conquista. Motivo: 5 exploradores contra aldeia de jogador defendida morrem sem relatório, e relatório que não chega faz o alvo falhar em scout_deadline_missed',
    'pvp_conquest.scout_max_age_hours': 'Validade máxima do relatório de scout usado na conquista PvP (padrão: 24 horas). Relatório mais antigo é ignorado e um novo scout é solicitado.',
    'pvp_conquest.dynamic_moral_night_bonus': 'Calcula moral (por pontos atacante/defensor) e night bonus (horário + world settings) reais no simulador, em vez de valores neutros fixos (moral=100, sem night bonus). Estimativa best-effort — valide contra o simulador do próprio jogo antes de confiar totalmente (ver docs/backend.md)',
    'pvp_conquest.troops_home_ratio': 'Fração da população do exército das aldeias de origem (limpeza + nobres) que precisa estar EM CASA antes de o simulador julgar a operação. O farm e a coleta são suspensos nessas aldeias assim que o alvo é cadastrado, mas isso só impede sair mais tropa — a que já está voando leva horas para voltar. Sem este portão a viabilidade era decidida contra o que estivesse parado na aldeia naquele minuto (0.9 = 90%)',
    'pvp_conquest.max_troop_wait_hours': 'Teto de espera do estágio "aguardando tropa". Passado esse tempo o bot simula com o que tiver, em vez de esperar para sempre por tropa que pode ter morrido ou estar de apoio em outra aldeia (padrão: 6 horas)',
    'pvp_conquest.sim_lead_seconds': 'Antecedência mínima, em segundos, entre a simulação/agendamento e a primeira saída obrigatória de um ataque. Quando o prazo chega a essa margem o bot para de esperar tropa e decide com o que tem — simulação atrasada não vira ataque nenhum, porque a operação já teria perdido a janela de saída (padrão: 1800 = 30 min)',
    'statue': 'Leitura periódica do estado do(s) Paladino(s) via screen=statue (Feature 24, fase 1 — só leitura, sem automação)',
    'statue.enabled': 'Ativar a leitura periódica do estado do Paladino (nível, XP, skills, slots) e persistência em cache/statue/status.json para exibição em /statue',
    'inventory': 'Leitura periódica do inventário (Perfil > Inventário) via screen=inventory (Feature 25, fase 1 — só leitura, nenhum item é ativado)',
    'inventory.enabled': 'Ativar a leitura periódica do inventário (itens, quantidades, descrições, validade) e persistência em cache/inventory/status.json para exibição em /inventory. Custa duas requisições por ciclo: o HTML da tela (rótulos de tipo/categoria) e o AJAX get_inventory (os itens)',
}

buildings = ["main", "barracks", "stable", "watchtower", "smith", "garage", "place", "statue", "market", "wood",
             "stone", "iron", "farm", "hide", "wall", "snob", "church"]

# Sections that contain nested dicts and need special rendering in the config UI
nested_sections = ["conquest", "resource_sharing", "inheritance", "empire", "watchtower", "profile_templates", "pvp_conquest"]
